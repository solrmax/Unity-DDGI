...
StructuredBuffer<LightField> LBuffer;
LightField L;

Texture2D irradianceTex;
SamplerState sampler_irradianceTex;
Texture2D weightTex;
...
float2 textureCoordFromDirection(float3 dir, int probeIndex, int fullTextureWidth, int fullTextureHeight, int probeSideLength) {
    ...
    uint probesPerRow = (fullTextureWidth - 2) / (uint)probeWithBorderSide;
...
{  
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        // Moment visibility test
        {
            ...
            float2 temp = weightTex.SampleLevel(sampler_weightTex, texCoord, 0).rg;
            float mean = temp.x;
        ...
        float3 probeIrradiance = irradianceTex.SampleLevel(sampler_irradianceTex, texCoord, 0).rgb;
